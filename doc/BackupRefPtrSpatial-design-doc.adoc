...
The reference count metadata for each allocation is stored prior to the
allocation within its slot. PartitionAlloc can locate the metadata for
...
image:media/BRPSpatial-ptr-zones.png[image,width=626,height=180]
...
inline extensions to BackupRefPtr that can be excluded by the
preprocessor when BACKUP_REF_PTR_CHECK_SPATIAL is defined as 0. It also
includes some minor extensions to PartitionAlloc.
...
zone, BackupRefPtrSpatial first subtracts the size of per-slot metadata
...
to range from the beginning of its referent’s data region to the end of
the metadata for the next allocation. Thus, by subtracting the size of
the metadata prior to mapping the shifted pointer to its slot
BackupRefPtrSpatial can identify the slot for that pointer’s authorized
referent, even when the pointer points into the metadata for the next
allocation. This ability is actually provided by the base BackupRefPtr
implementation. The detection of when the exclusion zone is entered is
implemented by checking whether the shifted pointer plus the addend
crosses either the lower or the upper slot boundary.
...
a new spatial safety violation, and code should not depend on an upcast
to avoid a spatial safety violation.
