...
for the preceding slot. The remainder of the reference count is in
the exclusion zone for the preceding slot.
...
Each poison zone permits pointers to range a distance equal to the size
of the cookie plus one byte beyond the usable space of the allocation to
match the maximum distance that is supported by BackupRefPtr in the
configuration that stores reference counts before allocations, which
is the stricter configuration.
...
the end of its trailing cookie. By subtracting the size of
